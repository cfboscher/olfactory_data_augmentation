...
def prepare_data_step_1(root_path, dataset):
    ...
    elif dataset == 'coco_aud':
        cleansed_path = os.path.join(root_path, "MS_COCO_Captions/auditory_dataset_step1.csv")
        df = pd.read_csv(cleansed_path)

    elif dataset == 'coco_olf':
        cleansed_path = os.path.join(root_path, "MS_COCO_Captions/olfactory_dataset_step1.csv")
        df = pd.read_csv(cleansed_path)

    elif dataset == 'coco_gus':
        cleansed_path = os.path.join(root_path, "MS_COCO_Captions/gustatory_dataset_step1.csv")
        df = pd.read_csv(cleansed_path)
...
def prepare_data_step_2(root_path, dataset):
    if dataset == 'odeuropa':
        ...
        cleansed_path = os.path.join(root_path, "MS_COCO_Captions/auditory_dataset_step2.csv")
        df = pd.read_csv(cleansed_path)
        df = df[~df['label'].isna()]
        df['label'] = df['label'].apply(lambda x: ast.literal_eval(x))
        df['token'] = df.apply(lambda x: ' '.join([y['text'] for y in x.label]), axis=1)
        ...
        cleansed_path = os.path.join(root_path, "MS_COCO_Captions/olfactory_dataset_step2.csv")
        df = pd.read_csv(cleansed_path)
        df = df[~df['label'].isna()]
        df['label'] = df['label'].apply(lambda x: ast.literal_eval(x))
        df['token'] = df.apply(lambda x: ' '.join([y['text'] for y in x.label]), axis=1)
        ...
        cleansed_path = os.path.join(root_path, "MS_COCO_Captions/gustatory_dataset_step2.csv")
        df = pd.read_csv(cleansed_path)
        df = df[~df['label'].isna()]
        df['label'] = df['label'].apply(lambda x: ast.literal_eval(x))
        df['token'] = df.apply(lambda x: ' '.join([y['text'] for y in x.label]), axis=1)
