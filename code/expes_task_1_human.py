...
def main():
    ...
    # 1) Load and split
    # df = pd.read_csv('code/symtesens_annotation/GPT4/source_gpt4/data_olf_gpt4_preprocessed.csv')
    # df = df.dropna(subset=["normalized_text","contains_ref"])
    # df["label"] = df["label"].astype(int)

    sizes = [20, 50, 75, 100, 150, 200, 250, 300, 400, 450, 500, 750, 1000, 1250, 1500, 1750]
    ...
    for size in sizes:
        ...
        try:
            ...
            train_df, test_df = pd.read_csv(f'data/preprocessed/train_fewshot_{size}.csv'), pd.read_csv(
            ...
            for fold, (train_idx, val_idx) in enumerate(skf.split(train_df, train_df["contains_ref"])):
                ...
                train_loader = get_data_loader(train_fold, BertTokenizer.from_pretrained(args.step1_model), norms)
                ...
                optimizer = AdamW(model.parameters(), lr=config.learning_rate_step1, eps=config.learning_rate_step2)
