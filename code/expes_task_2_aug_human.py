...
def main():
    ...
    for max_size in max_sizes:
        ...
        for size in sizes:
            ...
            if size <= max_size:
                print(f"===============================")
                print(f"STEP 2 - SIZE : {size} (OVER {max_size})")
                print(f"===============================")

                print('Splitting data...')
                train_data, test_data = pd.read_csv(f'data/preprocessed/train_fewshot_mixed_{str(size)}_odeuropa_aug_{str(max_size)}s2_shannon.csv'), pd.read_csv(
                ...
                for i in range(10):
                    ...
                    test_data['pred_token'] = test_data.apply(
                        lambda x: apply_heuristic(x.clean_sentence, x.pred_token, sensorimotor_norms, glove_models,
                                                  config), axis=1)
